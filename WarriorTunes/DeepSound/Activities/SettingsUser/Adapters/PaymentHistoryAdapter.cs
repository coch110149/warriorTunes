...
namespace DeepSound.Activities.SettingsUser.Adapters
{
    public class PaymentHistoryAdapter : RecyclerView.Adapter
    {
        ...
        public ObservableCollection<WithdrawalsDataObject> WithdrawalsList = new ObservableCollection<WithdrawalsDataObject>();
        ...
        public override int ItemCount => WithdrawalsList?.Count ?? 0;
        ...
        public override void OnBindViewHolder(RecyclerView.ViewHolder viewHolder, int position)
        {
            try
            {
                ...
                {
                    var item = WithdrawalsList[position];
                    if (item != null)
                    {
                        if (item.Id == "000")
                        {               
                        ...
                        {
                            holder.Amount.Text = "$" + item.Amount;
                            holder.Requested.Text = Methods.Time.TimeAgo(Convert.ToInt32(item.Requested), false);
        ...
        public WithdrawalsDataObject GetItem(int position)
        {
            return WithdrawalsList[position];
        }
