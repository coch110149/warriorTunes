...
namespace DeepSound.Activities.Comments.Adapters
{
    public class CommentsAdapter : RecyclerView.Adapter, ListPreloader.IPreloadModelProvider
    {
        ...
        public override void OnBindViewHolder(RecyclerView.ViewHolder viewHolder, int position)
        {
            try
            {
                if (viewHolder is CommentAdapterViewHolder holder)
                {
                    ...
                    {
                        if (Type == "Blog")
                            GlideImageLoader.LoadImage(ActivityContext, item.UserDataBlog?.Avatar, holder.Image, ImageStyle.CircleCrop, ImagePlaceholders.DrawableUser);
                        else
        ...
        void Click(CommentAdapterClickEventArgs args) => ItemClick?.Invoke(this, args);
        void LongClick(CommentAdapterClickEventArgs args) => ItemLongClick?.Invoke(this, args);
